...
const Article = (props) => {
    ...
    useEffect(() => {
        ...
        if (props.data.data.length > 0) {
            ...
            const convertToDOM = new DOMParser().parseFromString(props.data.data[0].article, "text/html")
                ...
                // ) : (
                    props.data.data.length > 0 ? (
                        <div className="relative">
                            <Head>
                                <meta property="og:image" content={`${props.data.data.img}?fit=max&amp;fm=jpg&amp;w=1000`} />
                            ...
                            <div className="relative">
                                <img src={props.data.data[0].img} alt={props.data.data[0].title} className="w-full rounded-md h-60 sm:h-72 md:h-96" />
                                ...
                                <div className="absolute h-full right-4 top-0">
                                    <div className={`overflow-hidden absolute right-0 -bottom-1 w-12 ${shareToggle ? 'h-60' : 'h-0'} transform translate-y-2 shadow-md bg-white rounded-md rounded-br-lg`} style={{transition: '.3s'}}>
                                        <SocialMediaShare url={'https://sidi.vercel.app/blog/'+router.query.url} title={props.data.data[0].title} />
                                    </div>
                            ...
                            <div className="flex items-center space-x-3 mt-4 pt-3 pb-2">
                                <div className="w-14 h-14 rounded-full">
                                    <img src={'/images/'+props.data.data[0].createdLogo} className="w-14 h-14 rounded-full" alt={props.data.data[0].cratedName} />
                                </div>
                                <div>
                                    <span className="block text-gray-700">Posted by <span className="font-medium text-indigo-700">{props.data.data[0].cratedName}</span></span>
                                    <span className="block text-gray-700">Posted at <span className="font-medium text-indigo-700">{new Date(props.data.data[0].date).toDateString()}</span></span>
                                </div>
                            </div>
